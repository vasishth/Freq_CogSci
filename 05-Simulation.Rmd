...
The correlation matrices and the subject/item random effects standard deviations are used to assemble the variance covariance matrix; this is done using the `sdcor2cov` function from the `SIN` package. For the variance covariance matrix for items random effects, we use an intermediate value of 0.5 for the correlation parameter because the linear mixed model was unable to estimate the parameter. 
...
## ## this won't work:
#Sigma_w<-SIN::sdcor2cov(stddev=item_ranefsd,corr=item_ranefcorr)
#solve(Sigma_w)
...
The terms u0, w0, and u1, w1, which are the adjustments to the intercepts and slopes by subject and item, are stored in two matrices that are generated randomly each time that we simulate new subjects/items. This is done by using the `mvrnorm` function in the MASS library. For example, given the variance covariance matrix for subjects `Sigma_u` that we created above, the subject random effects (intercept and slope adjustments) for 10 subjects can be generated in a matrix as follows:
...
c(mean(failed),mean(abs(sotvals)>2,na.rm=TRUE))
}
```
...
### Part 1 {#sec:SimulationexercisesPart1}
...
### Part 2 {#sec:SimulationexercisesPart2}
...
### Part 3 {#sec:SimulationexercisesPart3}

...
kbl(table4a,
  digits = 1, booktabs = T, position = "h", vline = "", # format="latex",
  caption = "Summary statistics per condition for the simulated data."
)
```
...
```{r table17, echo=FALSE, results="asis", eval = TRUE}
...
tab5 <- df_contrasts5 %>%
  group_by(F) %>%
  dplyr::summarize(M = mean(RT), SE = sd(RT) / sqrt(n()))
```

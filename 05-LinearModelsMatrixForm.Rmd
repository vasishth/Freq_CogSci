...
Y can be considered a point in n-dimensional space and the set of candidate $X\beta$ is a $p$-dimensional subspace of this space; see Figure \@ref(fig:leatsquares). There will be one point in this subspace which is closest to Y in terms of Euclidean distance. The unique $\beta$ that corresponds to this point is the **least squares estimator** of $\beta$; we will call this estimator $\hat \beta$.
...
### Estimating the parameters in a linear model {#sec:LMparamest}
...
X<-model.matrix(m1)
dim(X)
sigmahat<-summary(m1)$sigma
vcovmat<-sigmahat*solve(t(X)%*%X)
y<-as.matrix(crit$RT)
## will not work because of missing data 
hatbeta<-solve(t(X)%*%X)%*%t(X)%*%y

## second attempt:
## remove missing data:
crit<-subset(crit,RT!="NA")
m2<-lm(RT~Type,crit)
summary(m2)
X<-model.matrix(m2)
dim(X)
sigmahat<-summary(m2)$sigma
vcovmat<-sigmahat*solve(t(X)%*%X)
covariance<-vcovmat[1,2]
y<-as.matrix(crit$RT)
## will work because we removed the missing data 
hatbeta<-solve(t(X)%*%X)%*%t(X)%*%y
hatrho1 <- covariance/(sqrt(vcovmat[1,1])*sqrt(vcovmat[2,2]))
```
...
```{r echo=FALSE,eval=FALSE}
## set up sum contrast coding:
contrasts(crit$Type)<-contr.sum(2)
m3<-lm(RT~Type,crit)
summary(m3)
X<-model.matrix(m3)
dim(X)
sigmahat<-summary(m3)$sigma
vcovmat<-sigmahat*solve(t(X)%*%X)
covariance<-vcovmat[1,2]
y<-as.matrix(crit$RT)
hatbeta<-solve(t(X)%*%X)%*%t(X)%*%y
hatrho2 <- covariance/(sqrt(vcovmat[1,1])*sqrt(vcovmat[2,2]))
```
...
### Using ANOVA to carry out hypothesis testing  {#sec:LMManova}
...
### Computing ANOVA by hand {#sec:anovabyhand}
...
X1<-model.matrix(m1)
beta0%*%t(X0)%*%Y
beta1%*%t(X1)%*%Y-beta0%*%t(X0)%*%Y
t(Y)%*%Y - beta1%*%t(X1)%*%Y
## sum of squares
(t(Y)%*%Y - beta0%*%t(X0)%*%Y)/(p-q)
(t(Y)%*%Y - beta1%*%t(X1)%*%Y)/(n-p)
(beta1%*%t(X1)%*%Y-beta0%*%t(X0)%*%Y)/(p-q)
## F-value: (F(p-q,n-p)=F(1,1656))
((beta1%*%t(X1)%*%Y-beta0%*%t(X0)%*%Y)/(p-q))/((t(Y)%*%Y - beta1%*%t(X1)%*%Y)/(n-p))
```


### Generalized linear (mixed) model {#sec:GLMM}
...
library(lme4)
fm_skip<-glmer(skip ~ scale(word_complex,scale=FALSE)+scale(SC,scale=FALSE)+(1|subj)+(1|item),family=binomial(),hindi10)
summary(fm_skip)
fm_skip0WC<-glmer(skip ~ scale(SC,scale=FALSE)+(1|subj)+(1|item),family=binomial(),hindi10)
anova(fm_skip,fm_skip0WC)
fm_skip0SC<-glmer(skip ~ scale(word_complex,scale=FALSE)+(1|subj)+(1|item),family=binomial(),hindi10)
anova(fm_skip,fm_skip0SC)
alpha<-summary(fm_skip)$coefficients[1,1]
betawc<-summary(fm_skip)$coefficients[2,1]
betasc<-summary(fm_skip)$coefficients[3,1]
muhatwc<-alpha+betawc
muhatsc<-alpha+betasc
exp(muhatwc)/(1+exp(muhatwc))
exp(muhatsc)/(1+exp(muhatsc))
```

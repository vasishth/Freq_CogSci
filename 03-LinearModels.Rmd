...
It is an assumption of the linear model that the residuals are (approximately) normally distributed, That is what the statement $\varepsilon\sim Normal(0,\sigma)$ implies. When carrying out hypothesis testing, it is important to check that model assumptions are approximately satisfied; this is because the null hypothesis significance testing procedure requires approximate normality of residuals.
...
```{r}
(intercept_orig<-intercepts$`(Intercept)`[37])
(slope_orig<-slopes$so[37])
```
...
The estimates from subject 37 have become even more extreme once half the data from this subject have been deleted.

Now fit the linear mixed model and examine subject 37's estimates on undeleted vs. deleted data. Figure \@ref(fig:shrinkage2) illustrates the difference between the different models: there is hardly any difference.
...
abline(intercept_orig,slope_orig,lwd=4)
...
m1_lmer <- lmer(logrt ~ so + (1 + so || subject), gg05e1)
b0_orig<-summary(m1_lmer)$coefficients[1,1]
b1_orig<-summary(m1_lmer)$coefficients[2,1]
u<-ranef(m1_lmer)$subject
u0_orig<-u$`(Intercept)`
u1_orig<-u$so

abline(b0_orig+u0_orig[37],b1_orig+u1_orig[37],lty=2,col="gray",lwd=3)
...
The conclusion to take away from Figure \@ref(fig:shrinkage2) is that the estimates from the hierarchical model are almost unaffected by the missing data, but the estimates from the `lmList` model are heavily affected by the misestimation due to less data.
...
Linear mixed models as discussed in this chapter have been around for a long time [e.g., @henderson1959estimation]. Good introductions addressed to statisticians are @pinheirobates and @WoodGAMMs. There seem to be no good textbooks intended for non-statisticians, but interesting practical examples are in @faraway2016extending.

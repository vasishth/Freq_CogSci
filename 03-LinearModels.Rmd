...
In these data,  the parameter $\hat\sigma$ is estimated to be $213$ ms. How do we know what this estimate is? This parameter's estimate can be derived from the by-subject t-test output above. Let the mean difference between the means of the object and subject relative conditions be $\bar{d}$. The observed t-value is 
...
Now compute the difference between the predicted and observed values. These are the residuals.
...
One can visualize the correlation between intercepts and slopes by subjects; see Figure \@ref(fig:m3lmervarintscorrlsubj). The positive correlation of 0.58 between subject intercept and slope adjustments implies that slower subjects show larger effects. The correlation pattern isn't easy to see in Figure \@ref(fig:m3lmervarintscorrlsubj). 

```{r m3lmervarintslcorrsubj,echo=FALSE,fig.cap="The varying intercepts and varying slopes by subject in the model `m3_lmer`."}
...
The above example illustrates one important achievement of the linear mixed model: it conservatively down-weights estimates of extreme subjects' estimates towards the grand mean slope. This down-weighting is sometimes referred to as "borrowing strength from the mean". Figure \@ref(fig:shrinkage) illustrates the above discussion graphically.
...
```{r}
m1_lmer_orig<-lmer(log(rawRT)~so + (1+so|subject)+(1+so||item),gg05e1)
...
m4_lmer_del<-lmer(log(deletedRT)~so + (1+so|subject)+(1+so||item),gg05e1)

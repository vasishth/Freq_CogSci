...
```{r gg05e1plotdata,echo=FALSE,fig.cap="The distribution of the object and subject relative clause data at the critical region (relative clause verb) in the Grodner and Gibson 2005 data set (their Experiment 1)."}
p1<-ggplot(bysubj, aes(x=condition, y=rawRT)) + 
  geom_boxplot(outlier.colour=NA, width=.4) + 
  geom_dotplot(binaxis="y", stackdir="center",fill=NA)+
  xlab("The two conditions in Grodner and Gibson 2005, Expt 1")+
  ylab("Reading time (ms), by subjects")+theme_bw()

p2<-ggplot(byitem, aes(x=condition, y=rawRT)) + 
  geom_boxplot(outlier.colour=NA, width=.4) + 
  geom_dotplot(binaxis="y", stackdir="center",fill=NA)+
  xlab("The two conditions in Grodner and Gibson 2005, Expt 1")+
  ylab("Reading time (ms), by items")+theme_bw()
...
- Estimated mean object relative processing time: $\hat\beta_0=`r round(coef(m0raw)[1])`$ ms
.
- Estimated mean subject relative processing time: $\hat\beta_0+\hat\beta_1=`r round(coef(m0raw)[1])`+`r round(coef(m0raw)[2])`=`r round(coef(m0raw)[1])+(round(coef(m0raw)[2]))`$ ms.
...
- The intercept now represents the grand mean reading time: the estimate from the data is $\hat \beta_0=`r round(coef(m1raw)[1])`$.
- The estimated mean object relative reading time is: $\hat\beta_0+\hat\beta_1\times 1=`r round(coef(m1raw)[1])`+`r round(coef(m1raw)[2])`=`r round(coef(m1raw)[1])+round(coef(m1raw)[2])`$.
- The estimated mean subject relative reading time is: $\hat\beta_0+\hat\beta_1\times (-1)=`r round(coef(m1raw)[1])`-`r round(coef(m1raw)[2])`=`r round(coef(m1raw)[1])-round(coef(m1raw)[2])`$.
...
- The estimated grand mean reading time: $\hat\beta_0=`r round(coef(m1log)[1],4)`$.
- The estimated mean object relative reading time: $\hat\beta_0+\hat\beta_1=`r round(coef(m1log)[1],4)`+`r round(coef(m1log)[2],4)`=`r round(coef(m1log)[1],4)+round(coef(m1log)[2],4)`$.
- The estimated mean subject relative reading time: $\hat\beta_0-\hat\beta_1=`r round(coef(m1log)[1],4)`-`r round(coef(m1log)[2],4)`=`r round(coef(m1log)[1],4)-round(coef(m1log)[2],4)`$.
...
We return to our subject and object relative clause data from English [Experiment 1 of @grodner]. First load the data as usual, define relative clause type as a sum-coded predictor, and create a new column called `so` that represents the contrast coding ($\pm 1$ sum contrasts). From this point on, we will fit log-transformed reading times. To this end, create a column that holds log-transformed reading time. 
...
```{r bysubjrt,echo=FALSE,message=FALSE,fig.height=6,out.width='98%', fig.show = "hold", fig.cap = "The subject (coded -1) vs. object (coded +1) relative clause reading times for each subject. Also shown are linear models fit separately to the data for each subject."}
...
```{r}
ranefu0<-as.data.frame(ranef(m0_lmer, 
                             condVar = TRUE))
ranefu0SE<-transform(ranefu0, 
                     lwr = condval - 1.96*condsd, 
                     upr = condval + 1.96*condsd)
u0<-ranefu0SE$condval
u0sd<-ranefu0SE$condsd

b0<-summary(m0_lmer)$coefficients[1,1]
b0SE<-summary(m0_lmer)$coefficients[1,2]

b0u0<-b0 + u0
b0u0SD<-sqrt(b0SE^2 + u0sd^2)
b0u0lower<-b0u0 - 2*b0u0SD
b0u0upper<-b0u0 + 2*b0u0SD

bysubjintercept<-data.frame(b=b0u0,
                            lower=b0u0lower,upper=b0u0upper)
bysubjintercept$subj<-factor(1:42)
bysubjintercept<-
  bysubjintercept[order(bysubjintercept$b),]
bysubjintercept$subj<-
  factor(bysubjintercept$subj,
         levels=as.numeric(bysubjintercept$subj))
```


```{r bysubjvaryingintercepts,echo=FALSE,fig.cap="The predicted adjusted intercepts (ordered by magnitude) for each subject, with a 95 percent confidence interval."}
ggplot(bysubjintercept, aes(x=b, 
                            y=subj)) +
    geom_errorbar(width=.1, aes(xmin=lower, xmax=upper)) +
    geom_point(shape=21, size=3, fill="white") +
    ylab("subject id")+xlab("intercept estimates (log ms)")+theme_bw()
```
...
```{r varyingintercepts,echo=FALSE,fig.cap="A visualization of the linear mixed model with varying intercepts for each subject (black lines). The model is `m0_lmer`. Also shown as a gray line is the fitted line corresponding to the simple linear model `m0_lm`, without varying intercepts."}
a <- fixef(m0_lmer)[1]
...
print(dotplot(ranef(m1_lmer, condVar = TRUE))$subject)
```
...
```{r m3lmervarintscorrsubj,echo=FALSE,fig.cap="The varying intercepts and varying slopes by subject in the model `m3_lmer`."}
print(dotplot(ranef(m3_lmer, condVar = TRUE))$subject)
```
...
```{r m3lmervarintslsubj,echo=FALSE,fig.cap="The varying intercepts and varying slopes by subject in the model `m3_lmer`."}
...
```{r m3lmervarintslitem,echo=FALSE,fig.cap="Varying intercepts and varying slopes by item from the model `m3_lmer`."}
...
### Shrinkage of extreme estimates from individual subjects
...
### Shrinkage when data are missing
...
The estimates from subject 37 have become even more extreme once half the data from this subject have been deleted. The intercept goes from `r round(intercept_orig37,2)` in the original data to 
`r round(intercept_del37,2)` in the deleted data; and the slope  from `r round(slope_orig37,2)` in the original data to 
`r round(slope_del37,2)` in the deleted data.
...
Figure \@ref(fig:shrinkage2) illustrates the difference between the different models: there is hardly any difference between the two linear mixed model estimates; by contrast, the `lmList` estimates on the full vs. missing data differ quite a bit.

```{r shrinkage2,echo=FALSE,out.width='99%', fig.show = "hold", fig.cap = "The figure shows linear model fits (the grand mean estimates) for subject 37. When using lmList, deleting data leads to very different estimates; but using lmer, deleting half the data from this subject hardly affects the individual subject's estimates.", tidy = FALSE}
plot(jitter(gg05e1$so,.5),
        log(gg05e1$rawRT),axes=FALSE,
        xlab="Condition",ylab="rt (log ms)",
...
abline(intercept_del37,slope_del37)
## original data estimate lmList:
abline(intercept_orig37,slope_orig37,lwd=4)


## LMM on deleted data:
abline(b0_del+u0_del[37],b1_del+u1_del[37],
       lty=2,col="gray",lwd=1)
## LMM on full data:
abline(b0_orig+u0_orig[37],b1_orig+u1_orig[37],
       lty=2,col="gray",lwd=1)
...
The conclusion to take away from Figure \@ref(fig:shrinkage2) is that the estimates from the hierarchical model are almost unaffected by the missing data, but the estimates from the `lmList` model are heavily affected by the mis-estimation due to less data.
...
Linear mixed models as discussed in this chapter have been around since 1861 [@west2006linear]; thus, they were in use even before the t-test. Good introductions addressed to statisticians are @pinheirobates and @WoodGAMMs. There seem to be no good textbooks intended for non-statisticians, but interesting practical examples are in @faraway2016extending. @west2006linear strikes an excellent middle ground between practical examples and theoretical rigor, and will be accessible to non-statisticians who are comfortable with high-school level mathematics, and a passive familiarity of basic matrix algebra relevant for statistics [@fieller].

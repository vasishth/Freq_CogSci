...
We begin with the @grodner self-paced reading data we saw in the previous chapter. Load the data and compute the means by condition:
...
bysubj<-aggregate(rawRT~subject+condition,
                  mean,data=gg05e1)
byitem<-aggregate(rawRT~item+condition,mean,data=gg05e1)
...
We will discuss coding in  detail in a chapter later on, but right now the simple 0,1 coding above---called treatment contrasts---is enough for our purposes.
...
Now, the intercept is the mean of the subject relatives, and the slope is the difference between object and subject relative reading time. Note that  the  sign of the t-value has changed. 
...
These are estimates of the unknown values $\beta_0, \beta_1$.
...
It is an assumption of the linear model that the residuals are (approximately) normally distributed, That is what the statement $\varepsilon\sim Normal(0,\sigma)$ implies. As we will see later, it is important to check that model assumptions are approximately satisfied.
...
- Estimated mean object relative reading time: $\hat\beta_0+\hat\beta_1=`r round(coef(m1log)[1],4)`+`r round(coef(m1log)[2],4)`=`r round(coef(m1log)[1],4)+round(coef(m1log)[2],4)`$.
- Estimated mean subject relative reading time: $\hat\beta_0-\hat\beta_1=`r  round(coef(m1log)[1],4)`-`r round(coef(m1log)[2],4)`=`r  round(coef(m1log)[1],4)-round(coef(m1log)[2],4)`$.
...
The paired t-test has an equivalent in the linear modeling framework: the linear mixed model. We turn next to this extension of the simple linear model. The command corresponding to the   paired t-test in the  linear moodeling framework is:
...
You can find the proofs of the above assertions in books like [Rice](https://www.amazon.com/Mathematical-Statistics-Analysis-Available-Enhanced/dp/0534399428).
...
Let's generate some simulated data as an example. Assume that $\sigma = 1$,
...
Now let's compute the t-statistic using the above formula. Let the actual data be $x_1, x_2$.
...
The linear mixed model arises in the following situation. 

Suppose we have $i$ subjects and $j=1,2$ conditions. For simplicity, assume that each subject sees each condition once (e.g., the by-subjects aggregated English relative clause data), so we have two data points from each subject. In other words, the data are paired.
...
If we subtract the equation for conodition 2 from the equation for condition 1, the resulting equation is:
...
So, the paired t-test will deliver exactly the same t-score as the above linear model. 

Let's check this using our simulated data:
...
The t-statistic from the linear mixed model is exactly the same as the paired t-test.

## Linear mixed models

Now we are ready to look at linear mixed models in detail.

Returning to our SR/OR relative clause data from English (Grodner and Gibson, Expt 1). First we load the data as usual.  We define relative clause type as a sum coded predictor, and create a new column called `so` that holds  log-transformed reading time. 
...
Recall that these data have multiple measurements from each subject for each conditiono:
...
One thing to notice is that the coefficients (intercept and slope) of the fixed effects of the above model are identical to those in the linear model `m0` above. What is different between  the linear model and the linear mixed model is the standard error. 
...
#### The formal statement of the varying intercepts linear mixed model

The formal statement of varying intercepts linear mixed model is as follows; i indexes subjects, j items.

\begin{equation}
y_{ij} = \beta_0 + u_{0i}+(\beta_1)\times so_{ij} + \varepsilon_{ij}
\end{equation}

There are two variance components here:
...
We can have lmer fit different intercepts as well as different slopes for each subject. To achieve this, assume now that each subject's slope is also adjusted:
...
Again, i indexes subjects, j items.
...
Notice that subjects and items are fully crossed:
...
As usual, i indexes subjects, j items.
...
## complete pooling:
abline(lm(log(rawRT)~so,gg05e1),lwd=3,col="red")

...
```{theorem}
**ANOVA versus multiple regression**
```

```{r, echo=TRUE, message=FALSE, warning=FALSE}
# ANOVA: B_A(2) times B_B(2)
aov_car(DV ~ A*B + Error(id), data=df_contrasts4)
...
The results from the two analyses, shown in the R output and in Table <!--\@ref(tab:table16) and--> \@ref(tab:table17), are very different. 
...
tab5 <- df_contrasts5 %>% group_by(F) %>% summarize(M=mean(RT), SE=sd(RT)/sqrt(n()))
```

...
Next, we look at some perhaps subtle points about the t-test.
...
One unsurprising property of the averaged data is that the longer the average subject relative clause reading time for a particular subject, the longer that subject's average object relative clause reading time (Figure \@ref(fig:averageorsr)).
...
SRdatabyitem <- subset(byitem, condition == "subjgap")$rawRT
ORdatabyitem <- subset(byitem, condition == "objgap")$rawRT
...
You are given that the effect size is `r mu`, with 95\% confidence intervals \Sexpr{lowermu} and `r uppermu`. The standard deviation can range from `r lowersigma` to `r uppersigma`. 

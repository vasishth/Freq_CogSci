...
What does that sentence mean? We explain this point here because it is very helpful in understanding the absurdity of the p-value.
...
The distribution of the p-value is uniform---every value between 0 and 1 is equally likely. The practical implication of this criticism of p-values is that when we do a single experiment and obtain a p-value under the assumption that the null is true, if the null were in fact true, then we are just using a random number generator (generating a random number between 0 and 1, $Z\sim uniform(0,1)$) to make a decision that the effect is present or absent. Using the output of a $uniform(0,1)$ distribution to decide to reject the null hypothesis makes no sense at all. 

Here is the scientific procedure in R code, in the case where the null hypothesis is in fact true:
...
```{r}
summary_ttest<-function(res,paired=TRUE){
  ...
  print(paste("t(",dfs,")=",
              obs_t,sep=""))
  print(paste("p=",pval,sep=""))
  print(paste("estimate: ",est," [",ci[1],",",ci[2],"]",sep=""))
  } else {
    print(paste("t(",dfs,")=",
              obs_t,sep=""))
  print(paste("p=",pval,sep=""))
  print(paste("estimate 1: ",est[1],sep=""))
  print(paste("estimate 2: ",est[2],sep=""))
  print(paste("CI of difference in means: [",ci[1],",",ci[2],"]",sep=""))
  }
}  
```
...
In the data sets we will encounter in the remainder of this book, we will never use one or two-sample or paired t-tests. Instead, we will be using linear models or linear mixed models. The different varieties of t-test are discussed here because they are frequently used (and misused) in published papers.

Next, we look at some applications of the t-test and some subtle points relating to this statistical test.

## Using paired t-tests in complex factorial designs

In psychology and psycholinguistics, it is fairly common to run relatively complex factorial designs. An example is the $2\times 2$ repeated measures design in @levy2013expectation. Even more complex designs, like $2\times 2\times 2$ design, are widely used; an example is @fedorenko2006nature. We single out these two papers because the authors were exemplary in that they released their data on request.^[Research has repeatedly shown that relatively few researchers are willing to release their data. For example, @wicherts2011willingness report that ``Of the 49 corresponding authors, 21 (42.9%) had shared some data with Wicherts et al. Thirteen corresponding authors (26.5%) failed to respond to the request or any of the two reminders. Three corresponding authors (6.1%) refused to share data either because the data were lost or because they lacked time to retrieve the data and write a codebook. Twelve corresponding authors (24.5%) promised to share data at a later date, but have not done so in the past six years (we did not follow up on it). These authors commonly indicated that the data were not readily available or that they first needed to write a codebook.'' Interestingly, researchers refuse to release data even if the journal requires it; for example, after the *Journal of Memory and Language* instituted an obligatory data and code release policy, 10 out of 57 articles never released any data [@LaurinavichyuteVasishth2021].]
...
- Condition a: dative noun in main clause, adjunct in main clause.
- Condition b: dative noun in main clause, adjunct in subordinate clause.
- Condition c: dative noun in subordinate clause, adjunct in main clause.
- Condition d: dative clause in subordinate clause, adjunct in subordinate clause.

The research questions are: is there (i) a main effect of dative-noun location (main or subordinate clause), (ii) a main effect of adjunct location (main or subordinate clause), and (iii) is there an interaction between the two factors?

(ref:lk13E1) The experiment design in Experiment 1 of @levy2013expectation. The figure is re-used with permission from @VasishthMertzenJaegerGelman2018.

```{r lk13E1, fig.cap = "(ref:lk13E1)", out.width = "80%", echo = FALSE, fig.align = "center"}
knitr::include_graphics("figures/lk13E1.png", dpi = 1000)
```

Some pre-processing is needed to do the t-tests necessary to answer these questions. First, load the data, create a condition column, and then examine its structure:
```{r}
data("df_levykeller13E1")
head(df_levykeller13E1)
## create a condition column:
df_levykeller13E1$cond<-factor(ifelse(df_levykeller13E1$dat=="main" & df_levykeller13E1$adj=="main","a",
                               ifelse(df_levykeller13E1$dat=="main" & df_levykeller13E1$adj=="sub","b",
                        ifelse(df_levykeller13E1$dat=="sub" & df_levykeller13E1$adj=="main","c",
                               ifelse(df_levykeller13E1$dat=="sub" & df_levykeller13E1$adj=="sub","d","NA")))))
```

Notice that each of the 28 subjects sees each condition six times:

```{r}
t(xtabs(~subj+cond,df_levykeller13E1))
```

This is problematic for the t-test because the t-test assumes that each subject delivers exactly one data point per condition. The t-test requires that the data be aggregated: we have to restructure the data so that each subject's response for each of the four conditions is the mean of the six repetitions. This is often called a by-subject analysis:
...
The research questions (main effects and interactions of the two factors) can be visualized as in Figure \@ref(fig:lk13E1plot). 

```{r echo=FALSE, fig.cap="Visualizing the means from the four conditions in the Levy and Keller 2013 Experiment 1 design allows us to graphically summarize the statistical comparisons of interest: the main effects of the two factrs, and their interactions."}
meansE1<-with(bysubj,tapply(TFT,cond,mean))
names(meansE1)<-c("dat:main, \n adj:main",
                  "dat:main, \n adj:sub",
                  "dat:sub, \n adj:main",
                  "dat:sub, \n adj:sub")
...
The main effect of dative is asking whether the average of the two dat:main conditions is different from the average of the two dat:sub conditions. The null hypothesis is then:

\begin{equation}
H_{0,MEdat}: \frac{\mu_a + \mu_b}{2} = \frac{\mu_c + \mu_d}{2}
\end{equation}

The paired t-test can be used to test this main effect:
...
res<-t.test(mean_ab,mean_cd,paired=TRUE)
summary_ttest(res)
```
...
res<-t.test(mean_ac,mean_bd,paired=TRUE)
summary_ttest(res)
```

Finally, the interaction of the two factors (call it $dat\times adj$) is asking whether the adjunct effect within dat:main is different from the adjunct effect in dat:sub; in other words, the interaction is a difference between two differences:

\begin{equation}
H_{0,dat\times adj}: (\mu_a - \mu_b) = (\mu_c - \mu_d)
\end{equation}

The interaction is therefore testing a difference between two differences.  The implementation of the t-test works like this:

```{r}
diff_ab<-cond_a-cond_b
diff_cd<-cond_c-cond_d
res<-t.test(diff_ab,diff_cd,paired=TRUE)
summary_ttest(res)
```
...
@fedorenko2006nature report a self-paced reading study with eight conditions. 
...
The paired t-test assumes that each pair of data points that goes into the t-test is independent of the other pairs of data points. This implies that the data-frame cannot have more than one row for a particular pair. In other words, the data-frame cannot have repeated measurements spread out across rows.  
...
```{r}
## incorrect t-test!
t.test(rawRT ~ condition, df_gg05e1)
```
...
t.test(rawRT ~ condition, paired = TRUE, df_gg05e1)
```
...
t.test(rawRT ~ condition, byitem, paired = TRUE)
```
...
abline(lm(ORdatabyitem) ~ log(SRdatabyitem))
...
In order to take this variability into account, we must switch to the linear mixed model. The next chapter develops this more general framework.


### The difference between a significant and a non-significant result need not itself be significant

A very common error in psychology and psycholinguistics is to find a significant effect in one study, then find a non-significant effect in another study that changes one variable from the first study. The conclusion then drawn is that the variable that differs between the two studies is  ``significant.''  @gelmanhill07 summarize this mistake as ``the difference between significant and non-significant is itself not significant.'' Indeed, @nieuwenhuis2011erroneous shows that this mistake is widespread in areas like neuroscience. A real-live example will illustrate the problem.

@levy2013expectation published a paper in the *Journal of Memory and Language* in which they carried out two experiments. Experiment 1, which had a relatively simple syntactic structure, they had a $2\times 2$ repeated measures design in which a significant interaction was found, but in Experiment 2, which had a more complex syntactic structure, the interact was not significant. The conclusion was that the syntactic complexity in Experiment 2 was responsible for the switch from a significant effect in Experiment 1 vs. the non-significant effect in Experiment 2. Such a claim can be made only if the difference in the interaction between Experiments 1 and 2 is tested for.
...
\begin{enumerate}
\item
What will be the statistical power if the effect size is `r mu`, sample size is `r n` and standard deviation is `r uppersigma`?
\item
What sample size is needed to obtain a statistical power of 0.80 when the effect size is `r mu`,  and standard deviation is `r uppersigma`? 
\item Suppose now that Type I error probability is changed to 0.005. What sample size is needed to obtain a statistical power of 0.80 when the effect size is `r mu`, and standard deviation is `r uppersigma`? 
\end{enumerate}

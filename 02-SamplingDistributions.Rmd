...
```{r simt,fig.cap="The distribution of t-values under repeated sampling."}
n<-1000
nsim<-10000
tvals<-rep(NA,nsim)
for(i in 1:nsim){
...
plot(sds,lowess(power_upperval)$y,type="l",lty=2,
     ...
     main="Power curve (n=10) \n using simulation",ylim=c(min(power_lowerval),
                                                          max(power_upperval)))
...
The data are loaded and pre-processed as follows. The code below gives you the reading times for the full data-set. You will have to work out how to obtain mean or total reading times for the whole sentence in each condition.

```{r}
## load data:
data("df_gg05e1_full")
gg05e1_full<-df_gg05e1_full
## get data from relative clause verb:
gge1crit <- subset(gg05e1_full, ( condition == "objgap" & 
                             word_position == 6 ) |
